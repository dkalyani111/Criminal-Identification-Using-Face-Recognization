...
from tkinter import *   #gui designing
...
global var
import TrainModule     #module which we have created
...
def main_screen():
    ...
    C= Canvas(screen3)
    filename = ImageTk.PhotoImage(Image.open('E:/186040.gif'))
    background_label = Label(screen3, image=filename)
    C.pack()
...
#c = conn.cursor()
    ...
    cap = cv2.VideoCapture(0)   #1 for usb camera
...
def add_police_station():
    ...
    clear_button = Button(screen3, text="Clear All Data",  height=1, width=20,bg="black",fg="Red",font=("Arial Bold", 10),command=clear_all)
    clear_button.pack()
...
def clear_all():
    ...
    for f in filelist:
        os.remove(f)
    print("All record are cleared...")
    dbConnection.truncate_all()
...
def add_police_record():
    ...
    latitude=lat__log.get()
    longitude=long__log.get()
    email=email_log.get()
    mobile=mobile_log.get()
    insert_police_record(head_name,address,latitude,longitude,email,mobile)
